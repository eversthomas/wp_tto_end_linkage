/* global wp_tto_end_linkageIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'wp_tto_end_linkage-supports-dark-theme' ) ) {
	wp_tto_end_linkageDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function wp_tto_end_linkageDarkModeEditorInit( attempt ) {
	...
	if ( wp_tto_end_linkageIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						wp_tto_end_linkageDarkModeEditorInit( attempt + 1 );
					},
