...
class Twenty_Twenty_One_Dark_Mode {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'wp_tto_end_linkage_html_classes', array( $this, 'html_classes' ) );
	...
	public function editor_custom_color_variables() {
		...
		wp_enqueue_script(
			'wp_tto_end_linkage-dark-mode-support-toggle',
		...
		wp_enqueue_script(
			'wp_tto_end_linkage-editor-dark-mode-support',
			get_template_directory_uri() . '/assets/js/editor-dark-mode-support.js',
			array( 'wp_tto_end_linkage-dark-mode-support-toggle' ),
			'1.0.0',
			true
		);
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			'wp_tto_end_linkage-customize-controls',
			get_template_directory_uri() . '/assets/js/customize.js',
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'wp_tto_end_linkage-customize-helpers' ),
			'1.0.0',
			true
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'wp_tto_end_linkage' );
		}
		...
		$description .= sprintf(
			...
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'wp_tto_end_linkage' ),
			__( 'https://wordpress.org/support/article/twenty-twenty-one/', 'wp_tto_end_linkage' )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom right corner of the page.', 'wp_tto_end_linkage' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'wp_tto_end_linkage' ),
				'priority'        => 110,
	...
	public function admin_body_classes( $classes ) {
		...
		if ( $current_screen->is_block_editor() ) {
			...
			if ( $should_respect_color_scheme && Twenty_Twenty_One_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
				$classes .= ' wp_tto_end_linkage-supports-dark-theme';
			}
		}
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', 'wp_tto_end_linkage' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'wp_tto_end_linkage' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'wp_tto_end_linkage' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'Twenty Twenty-One uses LocalStorage when Dark Mode support is enabled.', 'wp_tto_end_linkage' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'wp_tto_end_linkage' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'wp_tto_end_linkage' );
