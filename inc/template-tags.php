...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			esc_html__( 'Published %s', 'wp_tto_end_linkage' ),
	...
	function twenty_twenty_one_posted_by() {
		if ( ! get_the_author_meta( 'description' ) && post_type_supports( get_post_type(), 'author' ) ) {
			...
			printf(
				...
				esc_html__( 'By %s', 'wp_tto_end_linkage' ),
...
if ( ! function_exists( 'twenty_twenty_one_entry_meta_footer' ) ) {
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', 'wp_tto_end_linkage' ) . '</p>';
			}
			...
			edit_post_link(
				sprintf(
					...
					esc_html__( 'Edit %s', 'wp_tto_end_linkage' ),
			...
			if ( has_category() || has_tag() ) {
				...
				$categories_list = get_the_category_list( __( ', ', 'wp_tto_end_linkage' ) );
				if ( $categories_list ) {
					printf(
						/* translators: %s: list of categories. */
						'<span class="cat-links">' . esc_html__( 'Categorized as %s', 'wp_tto_end_linkage' ) . ' </span>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}

				/* translators: used between list items, there is a space after the comma. */
				$tags_list = get_the_tag_list( '', __( ', ', 'wp_tto_end_linkage' ) );
				if ( $tags_list ) {
					printf(
						/* translators: %s: list of tags. */
						'<span class="tags-links">' . esc_html__( 'Tagged %s', 'wp_tto_end_linkage' ) . '</span>',
	...
	function twenty_twenty_one_the_posts_navigation() {
		...
		the_posts_pagination(
			array(
				...
				'before_page_number' => esc_html__( 'Page ', 'wp_tto_end_linkage' ),
				'mid_size'           => 0,
				'prev_text'          => sprintf(
					...
					sprintf(
						...
						esc_html__( 'Newer %s', 'wp_tto_end_linkage' ),
				...
				'next_text'          => sprintf(
					...
					sprintf(
						/* translators: %s: The post-type name. */
						esc_html__( 'Older %s', 'wp_tto_end_linkage' ),
						'<span class="nav-short">' . esc_html( $post_type_name ) . '</span>'
					),
