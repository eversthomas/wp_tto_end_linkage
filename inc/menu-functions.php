...
function twenty_twenty_one_add_sub_menu_toggle( $output, $item, $depth, $args ) {
	if ( 0 === $depth && in_array( 'menu-item-has-children', $item->classes, true ) ) {
		...
		// Add toggle button.
		$output .= '<button class="sub-menu-toggle" aria-expanded="false" onClick="wp_tto_end_linkageExpandSubMenu(this)">';
		...
		$output .= '<span class="screen-reader-text">' . esc_html__( 'Open menu', 'wp_tto_end_linkage' ) . '</span>';
		$output .= '</button>';
	}
	return $output;
}
